...
export const friendshipRequestRouter = router({
  send: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Check friendship request existed?
      const isFriendshipRequestExisted = await ctx.db
        .selectFrom('friendships')
        .select('id')
  ...
  accept: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.transaction().execute(async (t) => {
        ...
        // Check friendship request record with the opposite user existed?
        const isOppositeUserRequestExisted = await t
          .selectFrom('friendships')
          .select('id')
          .where('userId', '=', ctx.session.userId)
          .where('friendUserId', '=', input.friendUserId)
        ...
        // 2. Create a new friendship request record with the opposite user as the friend
        if (!isOppositeUserRequestExisted) {
          await t
            .insertInto('friendships')
            .values({
              userId: ctx.session.userId,
